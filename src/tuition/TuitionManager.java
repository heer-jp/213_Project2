...
public class TuitionManager {
    ...
    public static final int ADD_STUDENT_MIN_ARGUMENTS_COUNT = 3;
    public static final int ADD_STUDENT_ADDITIONAL_ARGUMENTS_COUNT = 1;
    public static final int ADD_STUDENT_MISSING_ARGUMENTS_COUNT = 2;
    ...
    public void run() {
        ...
        while (scanner.hasNextLine()) {
            ...
            if (input.hasMoreTokens()) {
                handleInput(roster, input);
            }
        }
    }

    /**
     * Calls the corresponding function to a given input.
     * Supports functionality to add, remove, and modify student tuition and status, as well as terminate the program.
     * @param roster roster to be modified by TuitionManager
     * @param input command provided by the user to modify roster
     */
    public void handleInput(Roster roster, StringTokenizer input) {
    ...
    private void addStudentToRoster(Roster roster, StringTokenizer input, String status) {
        ...
        switch (status) {
            case "AR":
                ...
                break;
            case "AN":
                studentToBeAdded = new NonResident(name, major, credits);
                break;
            case "AT":
                if (input.countTokens() != ADD_STUDENT_ADDITIONAL_ARGUMENTS_COUNT) {
                ...
                studentToBeAdded = new TriState(name, major, credits, state);
                break;
            case "AI":
                if (input.countTokens() != ADD_STUDENT_ADDITIONAL_ARGUMENTS_COUNT) {
                    System.out.println("Missing data in command line.");
                ...
                studentToBeAdded = new International(name, major, credits, studyingAbroad);
                break;
        }
     ...
     * @param input student information user input
     */
    private void setStudyAbroad(Roster roster, StringTokenizer input) {
     ...
     * @param input student information user input
     */
    private void setFinancialAid(Roster roster, StringTokenizer input) {
    ...
    /**
     * Parses user input arguments for adding a student.
     * @param input student information user input
     * @return String[] arguments if input is valid, null if not
     */
    private String[] parseAddStudentArguments(StringTokenizer input) {
        if (input.countTokens() < ADD_STUDENT_MISSING_ARGUMENTS_COUNT) {
            System.out.println("Missing data in command line.");
            return null;
        } else if (input.countTokens() < ADD_STUDENT_MIN_ARGUMENTS_COUNT) {
    ...
    private boolean checkFinancialAidAmountValidity(double financialAidAmount) {
        return financialAidAmount > MIN_VALID_AMOUNT && financialAidAmount < Resident.MAX_FINANCIAL_AID_AMOUNT;
    }
}
